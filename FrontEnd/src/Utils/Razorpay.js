...
export const handlePay = async (total, token, orderIds) => {
  try {
    ...
    const createOrdersResponse = await axios.post(
      'https://ecommerce-follow-along-judc.onrender.com/payment/create-order',
      {
        amount: total,
        currency: 'INR',
      }
    ...
    const responseKeys = await axios.get(
      'https://ecommerce-follow-along-judc.onrender.com/payment/get-razorpay-key'
    );
    // pay order
    ...
    const options = {
      ...
      handler: async function (response) {
        const result = await axios.post(
          `https://ecommerce-follow-along-judc.onrender.com/payment/pay-order?token=${token}`,
          {
            amount: amount,
